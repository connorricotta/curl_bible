...
from curl_bible.server_new import app
...
def test_colon_multi_verse():
    with TestClient(app) as client:
        response = client.get("/John:3:10-15")
        assert response.status_code == 200
        with open("curl_bible/tests/responses/colon_multi_verse.txt", "r") as f:
            sample_response = f.read()
            assert response.text == sample_response
...
def test_slash_multi_verse():
    with TestClient(app) as client:
        response = client.get("/John/3/10-15")
        assert response.status_code == 200
        with open("curl_bible/tests/responses/slash_multi_verse.txt", "r") as f:
            sample_response = f.read()
            assert response.text == sample_response
...
def test_query_single_verse():
    with TestClient(app) as client:
        response = client.get("?book=John&chapter=3&verse=10")
        compare(response, "query_single_verse.txt")


def test_query_multi_verse():
    with TestClient(app) as client:
        response = client.get("?book=John&chapter=3&verse=10-15")
